...
public class Kmeans extends Configured implements Tool {
	...
	public static class KmeansMapper extends
		...
		protected void setup(Context context) throws IOException,
			...
			Path centroids = new Path(conf.get("CENPATH"));//保存中心点向量
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			String[] iat = value.toString().split("/");
			for (String string : iat) {
				doc.put(Long.parseLong(string.split(":")[0]),//单词
						Double.parseDouble(string.split(":")[1]));//单词对应的TFIDF
			}
	...
	public static class KmeansReducer extends
		...
		@Override
		protected void setup(Context context) throws IOException,
				InterruptedException {//从HDFS中读取词表
			Configuration conf = context.getConfiguration();
